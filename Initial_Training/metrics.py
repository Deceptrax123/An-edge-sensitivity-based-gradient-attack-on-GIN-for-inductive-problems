...
def classification_multilabel_metrics(predictions, labels):
    acc = MultilabelAccuracy(num_labels=labels, average='weighted')
    f1 = MultilabelF1Score(num_labels=labels, average='micro')
    precision = MultilabelPrecision(num_labels=labels, average='micro')
    recall = MultilabelRecall(num_labels=labels, average='micro')
    auc = MultilabelAUROC(num_labels=labels, average='micro')
